namespace MoreEndingOptions.Patches
{
    ...
    [HarmonyPatch("JumpKing.GameManager.MultiEnding.NormalEnding.EndingBabe", "MakeBT")]
    public static class PatchEndingBabe
    {
        [UsedImplicitly]
        // ReSharper disable InconsistentNaming
        public static void Postfix(ISpriteEntity __instance, ref BehaviorTreeComp __result)
        {
            var path = Path.Combine(Game1.instance.contentManager.root, "ending", "custom_main_babe.xml");
            if (!File.Exists(path))
            {
                return;
            }
