namespace MoreBlockSizes.Patches
{
    ...
    public static class PatchDebugDraw
    {
        ...
        public static bool Prefix(LevelScreen __instance)
        {
            ...
            var sprite2 = sprite;
            var hitboxes = Traverse.Create(__instance).Field("m_hitboxes").GetValue<IBlock[]>();

            foreach (var block in hitboxes)
            {
                var draw_dst = block.GetRect();
                draw_dst = new Rectangle(draw_dst.X + 1, draw_dst.Y + 1, draw_dst.Width - 2, draw_dst.Height - 2);
                ...
                else if (block is SlopeBlock)
                {
                    switch ((block as SlopeBlock).GetSlopeType())
                    {
